package XAS::Web::Search;

our $VERSION = '0.01';

use XAS::Web::Profiles;
use XAS::Web::Profiles::Search;
...
sub init {
    ...
    $self->{'profile'} = XAS::Web::Profiles->new($search);
...
XAS::Web::Search - A class to build database queries.
...
=head1 SYNOPSIS
 ...
 my $search = XAS::Web::Search->new(-columns => \@fields);
...
the following:
 ...
 $params = {
     filter => qq/[{"field":"email", "type": "string", "value": "kevin@kesteb.us"}]/
 };
...
Here is an example were "value" can change from a simple string to something
 ...
 $params = {
     filter => qq/[{"field":"email", "type": "list", "value": ["kevin@kesteb.us","kevin@example.org"]}]/
 };
...
Copyright (C) 2016-2023 by Kevin L. Esteb
